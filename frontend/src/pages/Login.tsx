...
export default function Login({ onClose, onSuccess, isPage }: Props) {
  ...
  const content = (
    <div className="w-full max-w-md bg-white dark:bg-slate-800 rounded-2xl p-6">
      <h2 className="text-lg font-semibold mb-4 text-slate-900 dark:text-slate-100">Login</h2>
      <form onSubmit={submit} className="space-y-3">
        ...
        <div className="flex justify-end gap-2">
          ...
          <button type="submit" disabled={loading} className="px-4 py-2 rounded bg-cyan-600 text-white">{loading ? 'Signing in...' : 'Sign in'}</button>
        </div>
      </form>
    </div>
  )
