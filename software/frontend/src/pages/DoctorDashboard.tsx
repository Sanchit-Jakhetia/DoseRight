...
import { useDoctorOverview } from '../hooks/useApi'
...
export default function DoctorDashboard({ user }: Props) {
  ...
  const { data: overview, isLoading: loading, error: queryError } = useDoctorOverview(user?.token)
  const error = queryError ? (queryError as any)?.response?.data?.message || 'Failed to load doctor data' : null
